...
exports.XMLHttpRequest = function() {
  ...
  var defaultHeaders = {
    "User-Agent": "node-XMLHttpRequest-EN",
    "Accept": "*/*",
  };
  ...
  this.httpRequestCallback = function(responseType) {
    if (responseType == 'arraybuffer') {
      return this.httpBinaryResponseHandler;
    } else {
      return this.httpResponseHandler;
    }
  };

  this.httpResponseHandler = function(resp) {
    response = resp;
  ...
  this.httpBinaryResponseHandler = function(resp) {
    response = resp;
    var data = [], dataLen = 0;
    ...
    response.on('end', function() {
      if (sendFlag) {
        ...
        self.response = bufferToArrayBuffer(buf);
        setState(self.DONE);
        sendFlag = false;
      }
    });
  ...
  this.send = function(data) {
    ...
    if (settings.async) {
      ...
      // Create the request
      request = doRequest(options, this.httpRequestCallback(this.responseType)).on('error', function(error) {
        self.handleError(error);
      });
